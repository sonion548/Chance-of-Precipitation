...
try {
  const { id, done, failed } = await feedback.sendTest();
  const forwarded = done.filter((d) => d !== 'log');
  console.log(`\n  Report ${id} was accepted by: ${done.join(', ')}`);

  if (failed.length) {
    // The reason came from the provider itself and was printed above by the
    // logger — that line is the whole point of running this.
    console.log(`  REFUSED by: ${failed.join(', ')} — the reason is printed above.`);
    process.exitCode = 1;
  }
  if (forwarded.length) console.log(`  Go and look — it should be in your ${forwarded.join(' and ')} now.`);
  else if (!feedback.forwards) console.log('  Nothing is configured to forward it, so it only went to the log.');
  console.log('');
} catch (err) {
